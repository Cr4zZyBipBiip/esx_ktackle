	...
	while true do
        ...
        if PlayerData.job.name == PoliceJob then
            ...
            if IsControlPressed(0, Keys['LEFTSHIFT']) and IsControlPressed(0, Keys['G']) and not isTackling and GetGameTimer() - lastTackleTime > 10 * 1000 then
                ...
                if distance ~= -1 and distance <= Config.TackleDistance and not isTackling and not isGettingTackled and not IsPedInAnyVehicle(playerPed()) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
                    isTackling = true
